...
def cargar_series_temporales(file_path, variable_name):
    ...
    try:
        ...
        for col in df_series.columns:
            if col == 'date': # Columna índice, no necesita conversión
                continue
...
def procesar_jerarquia_geoespacial(shapefile_path, id_columna, output_prefix, gdf_estaciones, df_series, variable_code, var_config):
    ...
    run_annual_analysis_ppt = var_config['run_annual']

    # Carga y validación inicial del shapefile
    if not os.path.exists(shapefile_path):
        print(f"Advertencia: No se encontró {shapefile_path}. Saltando.")
        return
    try:
    ...
    # Filtrado de estaciones con datos para la variable actual
    ...
    # Join espacial y filtrado de polígonos
    ...
    if gdf_estaciones_con_jerarquia.empty:
        print(f"Advertencia: Ninguna estación CON DATOS de {variable_code} encontrada en {output_prefix}.")
        return
    ...
    # Cálculo y guardado de n_estaciones y GeoJSON
    ...
    try:
        ...
        print(f"ERROR guardando GeoJSON {geojson_path}: {e_geojson}")
        return

    # Procesamiento por polígono
    count_poligonos = 0
    for poly_id in ids_con_estaciones:
        # Define paths para limpieza
        json_path = os.path.join(OUTPUT_DIR, f"{output_prefix}_{poly_id}_{variable_code}.json")
        annual_ppt_path = os.path.join(OUTPUT_DIR, f"{output_prefix}_{poly_id}_ppt_annual.json") # Específico PPT
        annual_sd_max_path = os.path.join(OUTPUT_DIR, f"{output_prefix}_{poly_id}_sd_annual_max.json") # Específico SD
        annual_at_stats_path = os.path.join(OUTPUT_DIR, f"{output_prefix}_{poly_id}_at_annual_stats.json") # Específico AT
        
        # Limpia archivos previos por si este polígono falla o no genera datos
        for f_path in [json_path, annual_ppt_path, annual_sd_max_path, annual_at_stats_path]:
             if os.path.exists(f_path):
                 try: 
                     os.remove(f_path)
                 except OSError as e_rm: 
                     print(f"Advertencia: No se pudo eliminar archivo previo {f_path}: {e_rm}")
        ...
        try:
            ...
            if not columnas_validas:
                print(f"  Polígono {poly_id}: Sin columnas válidas. Saltando.")
                continue
            ...
            for col in columnas_validas:
                if (col, 'count') in monthly_agg.columns:
                    ...
                    if len(days_in_month) != len(count_values):
                        ...
                        condition = count_series.values < days_in_month.to_series(index=count_series.index).values * fill_rule_percent
                    else:
            ...
            # Preparar y guardar JSON Mensual
            ...
            for station_code in columnas_validas:
                ...
                try:
                    ...
                    station_info = pd.Series({'name': station_code, 'elevation': None})
            ...
            if run_annual_analysis_ppt and variable_code == 'ppt':
                ...
                for station_code in df_sum_monthly_ppt.columns:
                     ...
                     if climatologia_mensual.empty or climatologia_mensual.isnull().all(): 
                         continue
                ...
                # Aplicar reglas de validez PPT
                for station_code in df_annual.columns:
                    for year_start_date in df_annual.index:
                        year = year_start_date.year
                        try: 
                            num_meses_validos = monthly_counts_annual.loc[year_start_date, station_code]
                        except KeyError: 
                            num_meses_validos = 0
                        ...
                        if not isinstance(datos_del_ano_series, pd.Series):
                             if pd.api.types.is_scalar(datos_del_ano_series):
                                 ...
                                 if not original_index.empty: 
                                     datos_del_ano_series = pd.Series([datos_del_ano_series], index=[original_index[0]])
                                 else: 
                                     df_annual.loc[year_start_date, station_code] = None 
                                     continue
                             else: 
                ...
                # Guardar JSON Anual PPT si hay datos
                if not df_annual.dropna(how='all').empty:
                    ...
                    with open(annual_ppt_path, 'w', encoding='utf-8') as f:
                        json.dump(output_annual_json, f, ensure_ascii=False, indent=2)
                    print(f"    ✅ Guardado: {annual_ppt_path}")
                else:
                    print(f"  Polígono {poly_id}: Sin datos anuales PPT válidos.")
                    # No es necesario borrar aquí porque ya se hizo al inicio del bucle

            # B) Altura de Nieve (SD)
            elif variable_code == 'sd':
                print(f"  Polígono {poly_id}: Calculando máximo anual SD...")
                df_annual_max = df_agg_monthly.resample('YS').max()

                # Aplicar regla de validez SD (6 meses)
                ...
                if not df_annual_max.dropna(how='all').empty:
                    ...
                    for station_code in columnas_validas:
                        ...
                        output_annual_max_json[station_code] = {
                            'name': station_info['name'], 'elevation': elevation_value,
                            'years': df_annual_max.index.year.tolist(), 'values': values_with_nulls
                        }
                    with open(annual_sd_max_path, 'w', encoding='utf-8') as f:
                        json.dump(output_annual_max_json, f, ensure_ascii=False, indent=2)
            ...
            # C) Temperatura (AT)
                ...
                df_annual_stats = df_agg_monthly.resample('YS').agg(['max', 'min'])

                # Aplicar regla de validez AT (9 meses)
                for station_code in df_annual_stats.columns.levels[0]:
                     for year_start_date in df_annual_stats.index:
                         try: 
                             num_meses_validos = monthly_counts_annual.loc[year_start_date, station_code]
                         except KeyError: 
                             num_meses_validos = 0
                         if num_meses_validos < 9: # Regla AT
                             df_annual_stats.loc[year_start_date, (station_code, 'max')] = None
                             df_annual_stats.loc[year_start_date, (station_code, 'min')] = None

                # Guardar JSON Anual AT si hay datos
                if not df_annual_stats.dropna(how='all').empty:
                    output_annual_stats_json = {}
                    for station_code in columnas_validas:
                        if station_code not in df_annual_stats.columns.levels[0]: 
                            continue
                        try:
                            station_info = estaciones_ordenadas.set_index('code_internal').loc[station_code]
                            if isinstance(station_info, pd.DataFrame): 
                    ...
                    if output_annual_stats_json: # Solo guarda si hay algo que guardar
                        with open(annual_at_stats_path, 'w', encoding='utf-8') as f:
                            json.dump(output_annual_stats_json, f, ensure_ascii=False, indent=2)
            ...
            count_poligonos += 1
             ...
             # No es necesario borrar archivos aquí porque ya se hizo al inicio del bucle
             continue # Continúa con el siguiente polígono
...
def exportar_datos_estaticos():
    ...
    for var_code, config in VARIABLES_CONFIG.items():
        ...
        try:
            df_check = pd.read_csv(file_path, nrows=0, encoding='utf-8', sep=',', decimal='.')
             ...
             try:
                 df_check = pd.read_csv(file_path, nrows=0, encoding='cp1252', sep=',', decimal='.')
             except Exception as e_read:
                  print(f"Error leyendo header de {file_path}: {e_read}")
                  continue
    ...
    def get_vars_string(row):
        return '|'.join(sorted(station_vars.get(row['code_internal'], [])))
    ...
    except Exception as e_geojson_main:
         print(f"ERROR CRÍTICO al guardar {geojson_path}: {e_geojson_main}")
         return
    ...
    for var_code, config in VARIABLES_CONFIG.items():
        print("\n=======================================================")
        print("--- Iniciando procesamiento para variable: {var_code.upper()} ---")
        print("=======================================================")

        df_series = cargar_series_temporales(config['path'], var_code)
        if df_series is None or df_series.empty:
            print(f"No hay datos de series válidos para {var_code}. Saltando.")
            continue
        ...
        for code in df_series.columns:
            if code in stations_processed or code not in codes_in_gdf:
                continue
            series_to_export = df_series[[code]].copy().dropna()
            if series_to_export.empty:
                continue
            ...
            try:
                 ...
                 print(f"ERROR al guardar CSV {csv_path}: {e_csv}")
        print(f"✅ Exportados {count} archivos CSV para '{var_code}'.")

        # Procesar jerarquías geoespaciales
